...
def main(args=None):
    ...
    if args is None:
        ...
        class Args:
            ...
            dict = "Data/short_answers.json"
            rows = 5
            cols = 5
            min_len = 3
            max_len = 5
            seed = None  # <-- new random each run
            symmetry = "rot180"
            allow_repeats = False
    ...
    # If seed is None, generate a new random seed for this run
    ...
    # --- Export puzzle to a JSON file ---
    import os
    from datetime import datetime

    output_dir = "generated_crosswords"
    os.makedirs(output_dir, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"{output_dir}/crossword_{timestamp}.json"

    puzzle_data = {
        "blocks": blocks,
        "grid": grid,
        "across": [{"num": n, "row": r, "col": c, "word": w, "clue": cl} for n, r, c, w, cl in across],
        "down": [{"num": n, "row": r, "col": c, "word": w, "clue": cl} for n, r, c, w, cl in down],
        "seed": args.seed,
    }
